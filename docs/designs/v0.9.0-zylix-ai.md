...
export class ModelManager {
  ...
  private registry: Map<string, ModelInfo>;
  ...
  async listAvailable(): Promise<ModelInfo[]> {
    const response = await fetch('https://models.zylix.dev/registry.json');
    return response.json();
  }

  // ダウンロード済みモデル一覧
  async listDownloaded(): Promise<ModelInfo[]> {
    // ローカルファイルシステムをスキャン
  }
  ...
  async download(
    ...
    const destPath = path.join(this.modelsDir, `${modelId}.gguf`);
    ...
    // SHA256検証
    const data = new Uint8Array(chunks.flatMap(c => [...c]));
    const hash = await crypto.subtle.digest('SHA-256', data);
    const hashHex = Array.from(new Uint8Array(hash))
      .map(b => b.toString(16).padStart(2, '0'))
      .join('');

    if (hashHex !== info.sha256) {
