...
final class ZylixBridge: ObservableObject {
    ...
    func initialize() {
        ...
        if result == ZYLIX_OK {
            isInitialized = true
            refreshState()
            print("[Zylix] Core initialized, ABI version: \(zylix_get_abi_version())")
        } else {
            lastError = String(cString: zylix_get_last_error())
            print("[Zylix] Failed to initialize: \(lastError ?? "unknown")")
        }
    }
    ...
    func dispatch(_ event: ZylixEvent) {
        guard isInitialized else {
            print("[Zylix] Cannot dispatch: not initialized")
            return
        }
        ...
        if result == ZYLIX_OK {
            ...
            lastError = String(cString: zylix_get_last_error())
            print("[Zylix] Dispatch failed: \(lastError ?? "unknown")")
        }
    }
