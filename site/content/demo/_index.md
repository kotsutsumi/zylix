---
title: Live Demo
description: "Interactive demo of Zylix Todo app - Experience cross-platform UI development with Zig and WebAssembly"
---
...
<div class="demo-notice" style="background: linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%); color: white; padding: 1rem 1.5rem; border-radius: 8px; margin-bottom: 1.5rem; font-size: 0.9rem;">
  <strong>Preview Mode:</strong> This demo uses a JavaScript implementation to showcase the UI and interaction patterns. The production version will run native Zig code compiled to WebAssembly for optimal performance.
</div>

<div id="todo-demo" class="hx-mt-8 hx-p-6 hx-bg-gray-100 dark:hx-bg-neutral-800 hx-rounded-lg">
  <noscript>JavaScript is required to run this demo.</noscript>
  <div id="todo-loading" class="hx-text-center hx-py-8">
    <p>Loading demo...</p>
  </div>
  <div id="todo-app" style="display: none;">
    <!-- Todo App UI will be rendered here -->
...
<script>
// Simple JavaScript Todo implementation for demo
...
function initTodoApp() {
  let todos = [
    { id: 1, text: 'Learn Zig', completed: false },
    { id: 2, text: 'Build VDOM', completed: true },
    { id: 3, text: 'Create bindings', completed: false }
  ];
  let nextId = 4;
  let filter = 'all';
  let renderCount = 0;
  ...
  function render() {
    ...
    statsText.textContent = `${todos.length} Todos | ${renderCount} Renders | ${elapsed.toFixed(2)} ms`;
  }
...
- [Core Logic (Zig)](https://github.com/kotsutsumi/zylix/blob/main/core/src/todo.zig)
- [WASM Bindings](https://github.com/kotsutsumi/zylix/blob/main/core/src/wasm.zig)
- [Web Demo](https://github.com/kotsutsumi/zylix/tree/main/platforms/web)
