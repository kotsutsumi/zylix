...
- **Version**: 変更ごとに増える単調増加カウンタ。
- **Diff**: 変更点を示す差分マスク。
- **Store**: 現在/過去の状態を保持するジェネリックコンテナ。
...
- `zylix_init()` 前に状態を読むと null/初期値になります。
- コミットせずに状態を更新すると差分が反映されません。
- ABI の state ポインタを dispatch 後も保持すると無効化されます。
